...
setSparseSigma = function(sparseSigmaFile){
  ...
  sigmaMatListR = list(locations = locations,
                     values = values,
                     nSubj = nSubj)
  #(int r, arma::umat & t_locationMatinR, arma::vec & t_valueVecinR)
  #print(dim(locations))
  #print(nSubj)
  #print(length(values))
  #setSparseSigmaInCPP(nSubj, locations, values)    # check Main.cpp
...
SAIGE.Region = function(objNull,
  ...
  for(i in (indexChunk+1):nRegions){
    ...
    StatVec = outList$TstatVec[!is.na(outList$TstatVec)]
	...
	for(m in 1:length(maxMAFlist)){
		...
		Phi = wadjVarSMat[tempPos, tempPos]
		...
		out_SKAT_List = try(SKAT:::Met_SKAT_Get_Pvalue(Score = Score,
                              ...
                              silent = TRUE)
  		BETA_Burden = sum(wStatVec[tempPos])/(sum(diag(wadjVarSMat[tempPos, tempPos])))
                SE_Burden = abs(BETA_Burden/qnorm(out_SKAT_List$param$p.val.each[which(out_SKAT_List$param$rho == 1)]/2))
		...
		if(class(out_SKAT_List) == "try-error"){
			...
			BETA_Burden = NA
            		...
            		error.code = 0
			BETA_Burden = sum(wStatVec[tempPos])/(sum(wadjVarSMat[tempPos, tempPos]))
		...
		resultDF = data.frame(Region = regionName,
                                                    ...
                                                    max_MAF = maxMAFName,
                                                    Pvalue = Pvalue[1],
                                                    Pvalue_Burden = Pvalue[3],
                                                    Pvalue_SKAT = Pvalue[2],
	   ...
	   if(isCondition){
		if(traitType == "binary"){
			...
			G1tilde_P_G2tilde_Mat_scaled = diag(re_phi$scaleFactor) %*% (outList$G1tilde_P_G2tilde_Weighted_Mat) %*% diag(outList$scalefactor_G2_cond)
			VarInvMat_cond_scaled = diag(1/(outList$scalefactor_G2_cond)) %*% outList$VarInvMat_G2_cond %*% diag(1/(outList$scalefactor_G2_cond)) * (1/weightMat_G2_G2)
		        adjCondTemp = G1tilde_P_G2tilde_Mat_scaled %*% VarInvMat_cond_scaled	
		...
		out_SKAT_List = try(SKAT:::Met_SKAT_Get_Pvalue(Score = wStatVec[tempPos],
                                                         Phi = wadjVarSMat[tempPos, tempPos],
                                                         r.corr = r.corr,
                                                         method = "optimal.adj",
                                                         Score.Resampling = NULL),
                ...
                BETA_Burden = sum(wStatVec[tempPos])/(sum(wadjVarSMat[tempPos, tempPos]))
                        SE_Burden = abs(BETA_Burden/qnorm(out_SKAT_List$param$p.val.each[which(out_SKAT_List$param$rho ==
                          1)]/2))
		...
		resultDF$Pvalue_cond = Pvalue[1]
		resultDF$Pvalue_Burden_cond = Pvalue[3]
		resultDF$Pvalue_SKAT_cond = Pvalue[2]
		resultDF$BETA_Burden_cond = BETA_Burden
		resultDF$SE_Burden_cond = SE_Burden
	    }
		pval.Region = rbind.data.frame(pval.Region, resultDF)
	}
    }

    pval.Region$MAC = outList$MAC_GroupVec
    if(traitType == "binary"){
    pval.Region$MAC_case = outList$MACCase_GroupVec
    pval.Region$MAC_control = outList$MACCtrl_GroupVec
    }
    pval.Region$Number_rare = outList$NumRare_GroupVec
    pval.Region$Number_ultra_rare = outList$NumUltraRare_GroupVec
                         ...
                         markerInfo)
{
  ...
  #print(RegionAnnoHeader)
  #print(HeaderInRegionData)
...
mainRegion = function(genoType, genoIndex, annoIndicatorMat, maxMAFlist, OutputFile, traitType, n, P1Mat, P2Mat, isImputation, annolist, isCondition)
{
  ...
  print("OKKKKKK1")
  print(OutList$infoVec)
  print(OutList$markerVec)
  print(OutList$pvalVec)
